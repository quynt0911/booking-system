...
func (r *offTimeRepository) Create(offTime *model.OffTime) error {
    ...
    return r.db.QueryRow(query, offTime.ExpertID, offTime.StartDate,
        offTime.EndDate, offTime.Reason).
...
func (r *offTimeRepository) GetByExpertID(expertID int) ([]*model.OffTime, error) {
    ...
    for rows.Next() {
        ...
        err := rows.Scan(&offTime.ID, &offTime.ExpertID, &offTime.StartDate,
            &offTime.EndDate, &offTime.Reason, &offTime.CreatedAt)
        if err != nil {
            return nil, err
        }
...
//Lấy các off_times của expert tại một ngày cụ thể:
